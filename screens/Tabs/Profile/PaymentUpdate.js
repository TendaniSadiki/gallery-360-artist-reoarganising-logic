...
const PaymentUpdate = ({ navigation }) => {
  ...
  const writeUserData = async () => {
    try {
      await updateDoc(doc(FIRESTORE_DB, "paymentDetails", user.uid), {
        accountHolder,
        bankName,
        accountNumber,
        branchCode,
  ...
  const handleContinue = () => {
    if (validateForm()) {
      writeUserData(); // Save data
    }
  };

  // Skip Button - directly navigate to profile
  const handleSkip = () => {
    navigation.dispatch(StackActions.replace("Profile"));
  };
    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <TouchableOpacity style={styles.button} onPress={handleSkip}>
          <Text style={styles.smallerButtonText}>Skip</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
